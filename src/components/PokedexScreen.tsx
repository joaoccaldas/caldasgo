...
import { POKEMON_DATABASE, getSpecies, getPokemonImage, TYPE_COLORS } from '../data/pokemonDatabase';
        ...
        <div className="grid grid-cols-3 gap-2">
          {filteredGrid.map((species) => {
            ...
            return (
              <div
                key={species.id}
                onClick={() => caught && setSelectedSpeciesId(species.id)}
              ...
              >
                ...
                {caught && caught.length > 1 && (
                  <span className="absolute top-1 right-1.5 bg-slate-700 text-white font-bold text-[10px] px-1.5 rounded-full">
                    x{caught.length}
                  </span>
                )}
                ...
                {caught ? (
                  <div className="w-full h-full flex items-center justify-center p-3 mt-2">
                    <img
                      src={getPokemonImage(species.id)}
                      alt={species.name}
          ...
          >
             ...
             <div className="w-full flex justify-center mt-2 z-10 relative">
               <div className="bg-white px-4 py-1 rounded-full shadow-sm border border-slate-200 z-10 flex items-baseline gap-1">
                 ...
                 <span className="text-2xl font-black text-slate-800 tracking-tighter">{bestOwned.cp}</span>
               </div>
             </div>
             ...
             >
               <img src={getPokemonImage(selectedSpecies.id)} alt={selectedSpecies.name} className="w-[70%] h-full object-contain drop-shadow-2xl" />
             </motion.div>
